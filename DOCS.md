...
login({email: "FB_EMAIL", password: "FB_PASSWORD"}, (err, api) => {
    ...
    fs.writeFileSync('fbstate.json', JSON.stringify(api.getAppState())); //fbstate.json or appstate.json
});
```
...
login({appState: JSON.parse(fs.readFileSync('fbstate.json', 'utf8'))}, (err, api) => {
    if(err) return console.error(err); //fbstate.json or appstate.json
    // Here you can use the api
});
```
...
login({appState: JSON.parse(fs.readFileSync('fbstate.json', 'utf8'))}, async function(err, api) { //fbstate.json or appstate.json
    if (err) return console.error(err);
...
login({appState: JSON.parse(fs.readFileSync('fbstate.json', 'utf8'))}, (err, api) => { //fbstate.json or appstate.json
    if(err) return console.error(err);
