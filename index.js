...
/**
    * Developers: @KanzuWakazaki - @HarryWakazaki - @TanjiroKamado0806
    ** Ilang salita tungkol sa seguridad ng developer appstate.
    *! Pahayag na tinatanggihan ang responsibilidad para sa seguridad ng appstate encryption ng mga sumusunod na operating system: windows, Android, Linux operating system,.. (siguro repl.it?),
    *! dahil pribado ang mga operating system sa itaas (maliban sa rep.it kung hindi pagmamay-ari ng manloloko ang iyong account o mag-imbita ng link para sumali).
    *! Kung ang nanghihimasok ay nagmamay-ari ng computer, ang mga pribadong operating system na ito, ang seguridad ng fca na ito ay hindi magagarantiya ng 100% ng oras..
*/
    ...
    Data: new Object({
        ObjFastConfig: {
            ...
            "HTML": {   
                "HTML": true,
                "UserName": "Guest",
                "MusicLink": "https://drive.google.com/uc?id=1HtiKHPMSv337Y4PVVCJ7dKkehtwaObH0&export=download"
            }   
        },
        CountTime: function() {
...
/**
 * Nagbabalik ito ng isang string ng HTML code.
 * @param UserName - Ang username ng gumagamit
 * @param Type - Ang uri ng user, alinman sa "Libre" o "Premium"
 * @param link - The link to the music you want to play
 * @returns Isang HTML file
 */
...
function ClassicHTML(UserName,Type,link) {
    ...
    <body>
        <center>
            <marquee><b>FCA - TANJIRO - REMAKE</b></marquee>
            ...
            <footer class="footer">
                <div id="music">
                    ...
                    <br>Thanks For Using <b>Fca-Tanjiro-Remake</b> - From <b>John Arida</b> <3<br>
                </div>
            </footer>
            </div>
        </center>
...
try {
    ...
    mainPromise = utils
        ...
        Fetch('https://raw.githubusercontent.com/tanjirokamado0808/fca-tanjiro-remake/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
            const localVersion = JSON.parse(readFileSync('./node_modules/fca-tanjiro-remake/package.json')).version;
                if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                    log.warn("[ FCA-TANJIRO ] •",getText(Language.NewVersionFound,JSON.parse(readFileSync('./node_modules/fca-tanjiro-remake/package.json')).version,JSON.parse(res.body.toString()).version));
                    if (global.Fca.Require.FastConfig.AutoUpdate == true) { 
                        log.warn("[ FCA-TANJIRO ] •",Language.AutoUpdate);
                            try {
                                execSync('npm install fca-tanjiro-remake@latest', { stdio: 'inherit' });
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./node_modules/fca-tanjiro-remake/package.json')).version;
    console.clear();
